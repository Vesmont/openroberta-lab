define(["require", "exports", "blockly", "blockly/nepo.blockly.hiddenField", "utils/util", "utils/nepo.logger"], function (require, exports, Blockly, nepo_blockly_hiddenField_1, U, nepo_logger_1) {
    "use strict";
            ...
            this.init = function () {
                ...
                if (firstMode.question) {
                    this.appendDummyInput('ROW').appendField(Blockly.Msg['SENSOR_' + this.sensor.name + '_' + this.robot.toUpperCase()]
                        || Blockly.Msg['SENSOR_' + this.sensor.name] || U.checkMsgKey('SENSOR_' + this.sensor.name), 'SENSORTITLE').appendField(modes, 'MODE').appendField(ports, 'SENSORPORT').appendField(slots, 'SLOT').appendField(Blockly.Msg['SENSOR_IS_'
                        + firstMode.name]
                        || firstMode.name);
                }
                else {
                    this.appendDummyInput('ROW').appendField(Blockly.Msg["GET"]).appendField(modes, 'MODE').appendField(Blockly.Msg['SENSOR_UNIT_' + firstMode.unit]
                        ...
                        || Blockly.Msg['SENSOR_' + this.sensor.name] || U.checkMsgKey('SENSOR_' + this.sensor.name), 'SENSORTITLE').appendField(ports, 'SENSORPORT').appendField(slots, 'SLOT');
                }
                        ...
                        for (var i = 0; i < this.sensor.modes.length; i++) {
                            if (this.sensor.modes[i].name === option) {
                                ...
                                /*if (this.sensor.modes[i].ports) {
                                    ...
                                    input.appendField(new Blockly.FieldDropdown(portsList), 'SENSORPORT').appendField(new HiddenField(), 'SLOT');
                                }*/
                                // TODO: split these blocks into two, ANALOG and DIGITAL to completely get rid of this					
